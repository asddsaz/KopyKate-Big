...
(function() {
  var ItemList, Page, Promise, Text, Time, ZeroFrame, call_after_interval, calling, editor, last_time, left_menuify, seedbox, top_menuify, uploader, video_lister, video_playing, videobox,
    indexOf = [].indexOf,
...
// UnSanitize &amp; -> & in links
  ...
  ItemList = class ItemList {
    ...
    sync(rows, item_class, key) {
      var current_obj, item, k, len, results, row;
  ...
  left_menuify = class left_menuify {
    ...
    render() {
      var item_head_version, item_home, item_home_link, item_latest, item_latest_link, item_seedbox, item_seedbox_link, item_videobox, item_videobox_link, menu_left, menu_left_items;
      menu_left = $("<div></div>");
  ...
  video_lister = class video_lister {
    ...
    update() {
      var channel_name, init_url, order_actual, query, query_string_no_space;
      ...
      return Page.cmd("dbQuery", ["SELECT * FROM file LEFT JOIN json USING (json_id) " + query + " ORDER BY date_added DESC"], (res1) => {
        return Page.cmd("optionalFileList", order_actual, (res2) => {
          ...
          stats = {};
          console.log("ORDER MODE: " + this.order_by);
          if (this.order_by === "peer") {
            results = [];
            for (i = k = 0, len = res2.length; k < len; i = ++k) {
              row2 = res2[i];
            ...
            for (i = l = 0, len1 = res1.length; l < len1; i = ++l) {
              ...
              for (j = m = 0, len2 = res2.length; m < len2; j = ++m) {
                ...
                if (row1.file_name === optional_name && this.counter < this.max_videos) {
                  video_loaded = true;
                  this.print_row(row1);
                }
              }
              if (j === res2.length) {
                if (this.counter < this.max_videos && video_loaded === false) {
                  results1.push(this.print_row(row1));
                } else {
                  results1.push(void 0);
                }
